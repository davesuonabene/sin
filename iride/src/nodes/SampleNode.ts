...
import { fetchLibrary, findLibraryFileForSnapshot, updateLibraryBpm, type LibraryFile } from '../api';
...
export class SampleNode extends BaseNode {
        ...
        tabs: [
            {
                ...
                sections: [
                    ...
                    { type: 'fields', fields: ['start_beat', 'original_bpm', 'target_bpm', 'key', 'key_match_mode', 'key_match_algorithm', 'transpose', 'cents', 'stretch_mode'] }
                ]
            },
            {
                id: 'CHAIN',
    ...
    static fields: FieldSchema[] = [
        ...
        { key: 'original_bpm', label: 'Original BPM', type: 'float', default: 120, min: 20, max: 300, unit: 'BPM', tab: 'SAMPLE' },
        { key: 'target_bpm', label: 'Target BPM', type: 'float', default: 120, min: 20, max: 300, unit: 'BPM', tab: 'SAMPLE' },
        { key: 'key', label: 'Key', type: 'string', default: '', tab: 'SAMPLE' },
        {
            key: 'key_match_mode',
            label: 'Key Matching',
            type: 'radio',
    ...
    async syncMetadataFromLibrary(files?: LibraryFile[], preferredId?: number | string | null): Promise<LibraryFile | null> {
        ...
        if (Number.isFinite(bpm) && bpm >= 20 && bpm <= 400 && this.properties.original_bpm !== bpm) {
        ...
        if (this.properties.key !== libraryKey) this.updateProperty('key', libraryKey);
